...
import { EvaluationService, EvalQuery } from '../../../apps/api/src/retrieval/evaluation.service';
import { HybridRetrieverService } from '../../../apps/api/src/rag/hybrid-retriever.service';
...
describe('AC-4: bun run evaluate:retrieval exits with code 0 and stdout has table with query/precision@5', () => {
  it('exits with code 0', () => {
    const result = execSync('cd /Users/williamkhoo/workspace/subrina-coder/projects/koda/repos/koda && bun run evaluate:retrieval', {
      encoding: 'utf-8',
      timeout: 120_000,
    });
  ...
  it('stdout contains column headers including query and precision@5 or precisionAt5', () => {
    const output = execSync('cd /Users/williamkhoo/workspace/subrina-coder/projects/koda/repos/koda && bun run evaluate:retrieval', {
      encoding: 'utf-8',
      timeout: 120_000,
    });
...
describe('AC-6: CI threshold — below 0.70 avg causes non-zero exit', () => {
  it('process exits with non-zero code when precisionAt5_avg < 0.70', () => {
    ...
    try {
      execSync(
        'cd /Users/williamkhoo/workspace/subrina-coder/projects/koda/repos/koda && RAG_EVAL_PROJECT_ID=proj_eval_001 bun run evaluate:retrieval',
        {
          encoding: 'utf-8',
          timeout: 120_000,
