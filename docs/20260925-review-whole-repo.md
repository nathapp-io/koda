...
- ~~**Non-atomic VCS repo writes (ambient client)**~~ — **withdrawn after final review**: the earlier note claimed the repository's `db` getter (ambient `prisma.client`) means `applyMergedPrTransition`'s `txManager.run` wraps writes that do not actually join the transaction. That is wrong for the production wiring: `PrismaModule.forRoot({ transaction: true })` (`app.module.ts:57-61`) patches `PrismaService.client` with an AsyncLocalStorage-aware transaction proxy, so writes issued via `this.prisma.client` inside `txManager.run` DO join the ambient transaction. (The observation was an artifact of the test module's pass-through `ITransactionManager`.) Residual note only: a bare `PrismaClient` not constructed through `PrismaModule` would not join.
